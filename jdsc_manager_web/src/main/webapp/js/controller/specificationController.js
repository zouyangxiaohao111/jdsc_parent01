// 定义Controller
app.controller("specificationController",function($scope,$controller,$http){
    ...
    $scope.reloadList = function(){
        ...
        $http.get("../specification/findPage/"+$scope.paginationConf.currentPage+"/"+$scope.paginationConf.itemsPerPage).success(function(resp){
            if(resp.success){
                // 分页数据
    ...
    $scope.save = function(){
        ...
        // 发送post请求，保存数据
        $http.post("../specification/"+method,$scope.entity).success(function(resp){
            if(resp.success){
                // 重新刷新数据
                $scope.reloadList();
            }else{
    ...
    $scope.findOne = function(id){
        // 通过主键查询
        $http.get("../specification/findOne/"+id).success(function(resp){
            if(resp.success){
                // 赋值给entity变量
